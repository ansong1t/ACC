...
class SearchItemsViewModel(
...
) : PagingViewModel<SearchItemsViewState, SearchedItemEntryWithDetails, ObservePagedSearchedItems>(
    state = SearchItemsViewState(),
    pageSize = 8
) {
    ...
    override fun SearchItemsViewState.setLoaded(isLoaded: Boolean): SearchItemsViewState =
        copy(isLoaded = isLoaded)

    override fun SearchItemsViewState.setStatus(status: UiStatus): SearchItemsViewState =
        copy(status = status)
}
